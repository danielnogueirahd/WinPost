...
public interface ContatosRepositorio extends JpaRepository<Contatos, Long> {
    ...
    List<Contatos> findByDiaEMesAniversario(@Param("diaMesFormatado") String diaMesFormatado, @Param("empresa") Empresa empresa);
    
    // --- RELATÓRIOS ---
    // Filtro completo blindado para a empresa atual
    @Query("SELECT DISTINCT c FROM Contatos c LEFT JOIN c.grupos g WHERE "
            + "c.empresa = :empresa AND " // <-- BLINDAGEM AQUI
    ...
    // Filtro para a tela de listagem blindado
    @Query("SELECT DISTINCT c FROM Contatos c LEFT JOIN c.grupos g WHERE " +
           "c.empresa = :empresa AND " + // <-- BLINDAGEM AQUI
    ...
    // (O João pode existir na Empresa A e outro João com o mesmo email na Empresa B)
