...
public class RelatorioService {
	...
	public ByteArrayInputStream gerarRelatorioContatos(List<Contatos> contatos) {
		...
		try {
			...
			for (Contatos contato : contatos) {
				...
				adicionarCelulas(tabela, fontDados, String.valueOf(contato.getId()), contato.getNome(),
						contato.getEmail(), contato.getTelefone(), endereco, contato.getCidade(), contato.getEstado());
			}
	...
	public ByteArrayInputStream gerarEtiquetas(List<Contatos> contatos) {
		...
		// Margens ajustadas (30 nas laterais, 20 topo/base) para evitar corte na
		// impressora
	...
	// =========================================================================
	// NOVO: RELATÓRIO DE PRODUTIVIDADE DA AGENDA
	// =========================================================================
	public ByteArrayInputStream gerarRelatorioAgenda(List<com.projeto.sistema.modelos.EventoAgenda> eventos,
			String mesAno) {
		...
		try {
			...
			// Título
			Font fontTitulo = FontFactory.getFont(FontFactory.HELVETICA_BOLD, 18, Color.DARK_GRAY);
			Paragraph titulo = new Paragraph("Relatório de Produtividade da Agenda", fontTitulo);
			titulo.setAlignment(Element.ALIGN_CENTER);
			document.add(titulo);

			// Subtítulo
			Font fontSub = FontFactory.getFont(FontFactory.HELVETICA, 12, Color.GRAY);
			Paragraph subtitulo = new Paragraph("Resumo de atividades - Período: " + mesAno, fontSub);
			...
			PdfPTable tabela = new PdfPTable(3);
			tabela.setWidthPercentage(100);
			tabela.setWidths(new float[] { 1.5f, 2.0f, 4.5f }); // Largura das colunas (Data, Tipo, Título)

			// Cabeçalho da Tabela
			adicionarCabecalho(tabela, "Data", "Tipo de Evento", "Título / Descrição");

			// Preenchimento com os Dados da Agenda
			Font fontDados = FontFactory.getFont(FontFactory.HELVETICA, 10);
			java.time.format.DateTimeFormatter formatadorData = java.time.format.DateTimeFormatter
					.ofPattern("dd/MM/yyyy");

			for (com.projeto.sistema.modelos.EventoAgenda evento : eventos) {
				// Deixa o tipo mais amigável para leitura no papel
				String tipoFormatado = evento.getTipo().substring(0, 1).toUpperCase()
						+ evento.getTipo().substring(1).toLowerCase();

				adicionarCelulas(tabela, fontDados, evento.getData().format(formatadorData), tipoFormatado,
						evento.getTitulo());
			}
